...
const Step1 = ({
  ...
  const [step1Data, setStep1Data] = useState<EventWithFeatures | null>(null);
  ...
  useEffect(() => {
    ...
    if (savedStep1Data) {
      ...
      setStep1Data(eventData); // Set the initial eventData if nothing is stored
    }
  }, [eventData]);

  const handleSaveAndContinue = () => {
    // Save current room data into localStorage before moving to the next step
    localStorage.setItem("step1Data", JSON.stringify(eventData));
      ...
      <Container>
        <div className="flex justify-between mb-3">
          <div>
            ...
            <div className="flex items-center gap-x-2">
              <p className="font-bold text-2xl">{eventData?.name}</p>
              <Badge>{eventData?.type}</Badge>
            </div>
          </div>
        ...
        <Carousel className="w-full mt-5">
          ...
          <CarouselContent className="md:-ml-1 ml-0">
            {eventData?.inclusions.map((amenity) => (
              <CarouselItem
                key={amenity.id}
        ...
        <div className="grid grid-cols-1 mt-5 gap-4">
          <div className="flex flex-col">
            ...
            <p className="text-sm">Status: {eventData?.status}</p>
            <ul>
