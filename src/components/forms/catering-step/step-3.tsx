...
import { getCateringAppointment } from "@/actions/catering";
...
const Step3 = ({
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      const data = await getCateringAppointment();
      if (data.cateringAppointment) {
        setAllEvents(
          data.cateringAppointment.map((event: any) => ({
            title: `Catering: ${event.room}, Guests: ${event.guest}`,
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    setAllEvents((prevEvents) => [
      ...prevEvents,
      {
        title: `Catering: ${newEvent.room}, Guests: ${newEvent.guest}`,
        ...
        <div className="grid grid-cols-2">
          <form className="space-y-3" onSubmit={handleFormSubmit}>
            <div className="space-y-1">
              ...
              <Input type="text" value={cateringData?.name} readOnly />
            </div>
            <div className="space-y-1">
